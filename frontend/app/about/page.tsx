...
export default function AboutPage() {
  return (
    <PageShell flushFooter footerTone="dark">
      ...
      >
        <Reveal className="shell">
          ...
          {/* Full width, and `text-display-xl` rather than `text-display`.
              ...
              below it. */}
          <h1 className="reveal font-display mt-8 text-display-xl">
            {about.headline}
          </h1>

          <div className="mt-12 grid gap-x-8 gap-y-10 lg:grid-cols-12 lg:items-start">
      ...
      <section id="about-work" className="tone-dark bg-carbon py-section-lg">
        <Reveal className="shell">
          ...
          <p className="reveal text-fg max-w-[52ch] text-lead">{about.body}</p>
      ...
      {/* ---------- The two of us ---------- */}
      <section id="about-us" className="tone-dark bg-carbon pb-section-lg">
        <Reveal className="shell">
          <div className="grid items-center gap-16 lg:grid-cols-12 lg:gap-8">
            ...
            {/* The cards are absolutely positioned inside this box, so it has
                to carry its own height. Sized to leave room for the stack's
                offsets rather than only the front card. */}
            <div className="reveal lg:col-span-6 lg:col-start-7">
              <CardSwap className="h-[420px] sm:h-[480px]">
                {about.founders.cards.map((card) => (
                  <SwapCard
                    key={card.caption}
                    caption={card.caption}
                    className="h-[280px] w-[220px] sm:h-[320px] sm:w-[260px]"
                  />
                ))}
              </CardSwap>
            </div>
